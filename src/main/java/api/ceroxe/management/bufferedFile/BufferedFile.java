package api.ceroxe.management.bufferedFile;

import api.ceroxe.exceptions.FileNotFoundException;
import api.ceroxe.exceptions.IncorrectTypeException;
import api.ceroxe.thread.ThreadManager;
import api.ceroxe.utils.ArrayUtils;
...
public class BufferedFile extends File {
    ...
    private static CopyOnWriteArrayList<BufferedFile> fileBox = new CopyOnWriteArrayList();
                ...
                for (String prefix : exceptionPrefix) {
                    if (!files.getAbsolutePath().toLowerCase().contains(prefix.toLowerCase())) {
                        Runnable runnable = new Runnable() {
                            @Override
                            public void run() {
                                result.addAll(files.getAllFiles());
                            }
                        };
    ...
    protected static void getAllFiles(File dir) {
        ...
        try {
            if (this.exists()) {
                if (this.isFile()) {
                    ...
                    CopyOnWriteArrayList<BufferedFile> files = this.getAllFiles();
